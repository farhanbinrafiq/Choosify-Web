...
export function CategoryPhotoCard({
  ...
  const cardClass = cn(
    ...
    isExpanded
      ? 'border-[#FF5B00] ring-4 ring-[#FF5B00]/5 z-20 shadow-md'
      : 'border-[#e8edf2] hover:border-[#FF5B00]/25 hover:shadow-sm',
    className,
  );
  ...
  const content = (
    <>
      <div className="relative w-full aspect-[4/3] bg-[#F0F4F9] overflow-hidden">
        ...
        />
        {isExpanded && (
          <div className="absolute inset-0 bg-[#FF5B00]/10 border-2 border-[#FF5B00]/40 pointer-events-none" />
        )}
      </div>

      <div className="w-full px-3 py-3 sm:px-3.5 sm:py-3.5 border-t border-[#e8edf2] bg-white">
        <h4 className="font-semibold text-[11px] sm:text-xs text-[#1a1a2e] group-hover:text-[#EF3C23] transition-colors leading-snug uppercase tracking-tight line-clamp-2">
          {name}
        </h4>
        <p className="text-[10px] text-[#FF5B00] font-semibold leading-none uppercase font-mono mt-1.5">
      ...
      {isExpanded && (
        <motion.div
          ...
          className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-4 h-4 bg-white rotate-45 border-r border-b border-[#FF5B00]/30 z-10"
        />
      )}
    </>
  );
