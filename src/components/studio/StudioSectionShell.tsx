...
export function StudioSectionShell({
  ...
  return (
    <div
      ...
      style={style}
      className={cn('relative scroll-mt-36 w-full', editMode && 'ring-1 ring-transparent hover:ring-[#FF5B00]/25 rounded-[5px]', className)}
    >
      {editMode && (
        <button
          type="button"
          onClick={() => onEdit?.(section.id)}
          className="absolute top-3 right-3 z-20 inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-[#1A1D4E] text-white text-[9px] font-black uppercase tracking-wider shadow-md hover:bg-[#EF3C23] transition-colors"
        >
          <Pencil size={11} />
          Edit {section.label}
        </button>
