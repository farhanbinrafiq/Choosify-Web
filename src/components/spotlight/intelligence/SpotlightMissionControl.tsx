  ...
  return (
    <div className="bg-white border border-[#e8edf2] rounded-xl p-4 flex flex-col min-h-[180px]">
      <div className="flex items-center justify-between mb-3">
        ...
        {href && (
          <Link to={href} className="text-[9px] font-bold text-[#E8500A] hover:underline uppercase">
            View all
          </Link>
        )}
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#050514] text-white rounded-xl p-5 flex flex-wrap items-center justify-between gap-4">
        ...
        <Activity className="h-10 w-10 text-[#E8500A] opacity-80 shrink-0" aria-hidden />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <Panel title="Best Performing" icon={<Trophy size={12} className="text-[#E8500A]" />} href="/marketing/intelligence/content">
        ...
        <Panel title="Trending Now" icon={<TrendingUp size={12} className="text-[#E8500A]" />} href="/marketing/intelligence/discovery">
        ...
        <Panel title="Commerce Drivers" icon={<ShoppingBag size={12} className="text-[#E8500A]" />} href="/marketing/intelligence/campaigns">
          {data.commerceDrivers.map((item) => (
            <Row key={item.label} label={item.label} value={`${item.clicks} clicks`} href={item.href} />
          ))}
        </Panel>

        <Panel title="Top Creators" icon={<Star size={12} className="text-[#E8500A]" />} href="/marketing/intelligence/creators">
