  ...
  return (
    <div className="bg-white border border-[#e8edf2] rounded-xl p-4">
      <div className="flex justify-between items-center mb-4">
        ...
        <span className="text-xs font-bold text-[#FF5B00]">{completed}/{items.length} · {pct}%</span>
      </div>
      <div className="h-1.5 bg-gray-100 rounded-full mb-4 overflow-hidden">
        <div className="h-full bg-[#FF5B00] rounded-full transition-all" style={{ width: `${pct}%` }} role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} />
      </div>
        ...
        {items.map((item) => (
          <li key={item.id} className="flex items-start gap-2">
            ...
            {item.href && !item.completed ? (
              <Link to={item.href} className="text-xs text-navy hover:text-[#EF3C23] font-medium">{item.label}</Link>
            ) : (
