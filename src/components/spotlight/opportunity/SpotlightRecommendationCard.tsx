...
export function SpotlightRecommendationCard({ opportunity, estimatedTotalImpact, missingCount }: SpotlightRecommendationCardProps) {
  ...
  if (opportunity) {
    return (
      <div className="bg-[#050514] text-white rounded-xl p-5 space-y-3">
        <p className="text-[10px] font-bold uppercase text-[#FF5B00]">Recommended next step</p>
  ...
  return (
    <div className="bg-gradient-to-br from-[#050514] to-navy text-white rounded-xl p-5 space-y-4">
      <p className="text-[10px] font-bold uppercase text-[#FF5B00] tracking-widest">Publisher Success Coach</p>
      ...
      <div className="flex flex-wrap gap-2 pt-2 border-t border-white/10">
        {quickActions.map((action) => (
          <EmiActionButton
            ...
            compact
            className="bg-white/10 border-white/20 text-white hover:text-[#EF3C23]"
          />
        ))}
      </div>
    </div>
