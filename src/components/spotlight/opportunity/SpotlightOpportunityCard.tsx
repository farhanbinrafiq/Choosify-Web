...
export function SpotlightOpportunityCard({
  ...
  const inner = (
    <div className={cn(
      ...
      opportunity.status === 'pinned' ? 'border-[#FF5B00] ring-1 ring-[#FF5B00]/20' : 'border-[#e8edf2]',
      ...
      {showCoach && (
        <p className="text-xs text-gray-600 leading-relaxed border-l-2 border-[#FF5B00]/40 pl-3">
          {opportunity.coachingMessage}
        </p>
      )}

      <div className="flex flex-wrap items-center gap-2 pt-1">
        <span className="text-[10px] font-bold text-[#FF5B00] uppercase">{opportunity.suggestedAction}</span>
      ...
      {opportunity.status === 'open' && (onDismiss || onResolve || onPin) && (
        <div className="flex flex-wrap gap-2 pt-2 border-t border-[#e8edf2]">
          {onResolve && (
            <button type="button" onClick={(e) => { e.preventDefault(); onResolve(opportunity.opportunityId); }} className="text-[9px] font-bold uppercase px-2 py-1 bg-[#FF5B00] text-white rounded">
              Resolve
            </button>
          )}
          {onPin && (
