...
export function ShoppableOverlay({ event, products, activeChapter, heroProductId }: ShoppableOverlayProps) {
  ...
  return (
    <aside className="space-y-4 text-left" aria-label="Shoppable products">
      {activeChapter && (
        <div className="p-3 rounded-[5px] bg-[#FFF8F4] border border-[#FF5B00]/20">
          <p className="text-[10px] font-black uppercase text-[#FF5B00]">{activeChapter.timestampLabel}</p>
          <p className="text-sm font-bold text-[#1a1a2e]">{activeChapter.title}</p>
        </div>
      )}

      {hero && (
        <div className="p-4 border border-[#FF5B00]/30 rounded-[5px] bg-white shadow-sm">
          <p className="text-[9px] font-black uppercase tracking-widest text-[#FF5B00] mb-2">Hero Product</p>
          ...
          <p className="text-lg font-black text-[#FF5B00] mt-1">৳{hero.price.toLocaleString()}</p>
          <div className="grid grid-cols-2 gap-2 mt-3">
            ...
            <button type="button" onClick={() => toggleWishlist(hero)} className="p-2 border rounded text-[#FF5B00] hover:text-[#EF3C23]" aria-label="Wishlist">
              <Heart
                size={14}
                strokeWidth={2}
                className={cn(
                  'text-[#FF5B00]',
                  savedProducts.some((p) => p.id === hero.id) && 'fill-[#FF5B00]',
                )}
              />
            </button>
            <button
              ...
              <li key={product.id}>
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-sm font-black text-[#FF5B00]">৳{product.price.toLocaleString()}</p>
                  </div>
                </Link>
              </li>
            ))}
      ...
      <div className="flex gap-2">
        <Link
          ...
          className="flex-1 text-center py-2 border border-[#e8edf2] text-[10px] font-bold uppercase rounded hover:border-[#FF5B00]/40"
        >
          View Brand
        </Link>
