    ...
    <div className={cn('space-y-6', className)} data-content-renderer="spotlight_universal">
      ...
      {content.publisher.publisherType === 'brand' && (
        <section className="flex items-center gap-3 p-4 bg-white border border-[#e8edf2] rounded-[5px]">
          ...
          {profileHref && (
            <Link
              to={profileHref}
              className="text-[10px] font-black uppercase tracking-widest text-[#FF5B00] hover:underline shrink-0"
            >
              View Brand
            </Link>
          )}
      ...
      {(primary || hasService) && (
        <section className="text-left bg-[#fafbfc] border border-[#e8edf2] rounded-[5px] p-4">
          <p className="text-[10px] font-black uppercase tracking-widest text-[#FF5B00] mb-3">
      ...
      <div className="flex flex-wrap gap-2">
        {primaryUrl && (
          <>
            <Link
              to={`${primaryUrl}#buy`}
              className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-[#FF5B00] text-white text-[10px] font-black uppercase tracking-wider rounded hover:bg-[#EF3C23] min-h-[44px]"
            >
              <ShoppingBag size={12} /> Shop Now
            </Link>
            <Link
              to={primaryUrl}
              className="inline-flex items-center gap-1.5 px-4 py-2.5 border border-[#e8edf2] text-[10px] font-bold uppercase tracking-wide text-[#1a1a2e] rounded hover:border-[#FF5B00]/40 min-h-[44px]"
            >
              Product Details
            </Link>
          </>
        ...
        {onWishlist && (
          <button
            type="button"
            onClick={onWishlist}
            className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-[#e8edf2] bg-white hover:border-[#FF5B00]/30 cursor-pointer min-h-[44px]"
          >
      ...
      {profileHref && content.publisher.publisherType === 'creator' && (
        <Link
          to={profileHref}
          className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-[#EF3C23]"
        >
