  ...
  const inner = (
    <>
      {publisher.logoUrl ? (
        ...
        <span
          className={cn(
            'rounded-full bg-[#FF5B00]/10 text-[#FF5B00] font-black flex items-center justify-center shrink-0',
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-1">
          ...
          {publisher.isVerified && !secondary && (
            <BadgeCheck size={compact ? 10 : 12} className="text-[#FF5B00] shrink-0" aria-label="Verified" />
          )}
        </div>
        {!compact && !secondary && (
