...
export function SpotlightCollectionCard({ collection, compact }: SpotlightCollectionCardProps) {
  return (
    <Link
      ...
      className="group flex flex-col h-full min-h-[120px] p-4 bg-white border border-[#e8edf2] rounded-[5px] hover:border-[#FF5B00]/40 hover:shadow-md transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF5B00]"
      aria-label={`Collection: ${collection.name}`}
    >
      <span className="text-[9px] font-black uppercase tracking-widest text-[#FF5B00]">{collection.kind}</span>
