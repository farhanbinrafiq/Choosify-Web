      ...
      <form onSubmit={handleSubmit} role="search" aria-label="Spotlight search">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 min-h-[48px] text-sm border border-[#e8edf2] rounded-[5px] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF5B00]"
                ...
                <div className="flex flex-wrap gap-2">
                  {items.map((s) => (
                    <button
                      ...
                      className="px-3 py-2 min-h-[44px] text-xs font-bold border border-[#e8edf2] rounded-full hover:border-[#FF5B00]/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF5B00]"
                    >
                      {s.label}
                    </button>
                  ))}
            ...
            <ul className="divide-y divide-gray-100 border border-[#e8edf2] rounded-[5px] overflow-hidden">
              {results.map((r) => (
                <li key={`${r.kind}-${r.entityId}`}>
                  <Link
                    to={r.href}
                    className="flex items-center justify-between gap-3 p-4 hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#FF5B00]"
                  >
                    <div className="text-left min-w-0">
                      <span className="text-[9px] font-black uppercase text-[#FF5B00]">{r.kind}</span>
