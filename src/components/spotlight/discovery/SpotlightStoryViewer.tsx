      ...
      <div className="relative flex-1 flex items-center justify-center px-4">
        ...
        <div className="max-w-lg w-full text-center space-y-4">
          ...
          {slide.href && slide.ctaLabel && (
            <Link
              to={slide.href}
              className="inline-flex px-6 py-3 min-h-[44px] bg-[#FF5B00] text-white text-xs font-black uppercase rounded"
            >
              {slide.ctaLabel}
            </Link>
          )}
    ...
    <div className="flex gap-3 overflow-x-auto pb-2" aria-label="Stories">
      {groups.map((g, i) => (
        <button
          ...
          type="button"
          onClick={() => onOpen(i)}
          className="shrink-0 flex flex-col items-center gap-1 min-w-[72px] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF5B00] rounded-lg p-1"
          aria-label={`Open story from ${g.publisherName}`}
        >
          <div className="w-16 h-16 rounded-full p-0.5 bg-gradient-to-tr from-[#FF5B00] to-amber-400">
            {g.publisherAvatar ? (
