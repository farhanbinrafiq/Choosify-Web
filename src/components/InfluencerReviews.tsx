...
function RenderAvatar({ avatar, name, size = 'sm' }: { avatar: string; name: string; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={`${sizeClass} rounded-full bg-[#FFF0E8] border border-[#FFD4BC] flex items-center justify-center font-bold text-[#FF5B00] shrink-0 uppercase`}>
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-2 px-4 text-center">
        <div className="w-11 h-11 rounded-full bg-[#FF5B00] text-white flex items-center justify-center shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:bg-[#FF6B00] shrink-0">
...
function CreatorReviewCard({
  ...
  return (
    <div
      ...
      data-format={variant}
      className="choosify-creator-hybrid-card ir-card flex flex-col bg-white h-full transition-all duration-200 rounded-[12px] border border-[#e8edf2] overflow-hidden shadow-[0_4px_16px_rgba(26,29,78,0.04)] hover:shadow-[0_8px_24px_rgba(255, 91, 0,0.08)] hover:border-[#FF5B00]/20"
    >
      <div
      ...
      <div className={`flex flex-col flex-grow text-left bg-white ${isPortrait ? 'p-3' : 'p-4'}`}>
        <span className="text-[9px] font-black text-[#FF5B00] tracking-[0.14em] uppercase mb-1.5">
          {review.category}
        </span>
...
export function InfluencerReviews({
  ...
  const renderSubtitleText = () => {
    if (!subtitle) {
      return (
        <>
          Trusted voices, <span className="text-[#FF5B00] not-italic">real opinions</span>
        </>
      );
    }
    if (subtitle.includes(',')) {
      ...
      return (
        <>
          {parts[0].trim()}, <span className="text-[#FF5B00] not-italic">{parts[1].trim()}</span>
        </>
      );
    }
    return subtitle;
    ...
    >
        ...
        <div className="flex flex-nowrap md:justify-center items-center gap-2 overflow-x-auto no-scrollbar w-full py-1.5 justify-start max-w-full select-none">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] sm:text-[11px] font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer border select-none whitespace-nowrap outline-none ${
                  isActive
                    ? "bg-[#FF5B00] text-white border-[#FF5B00] shadow-sm shadow-[#FF5B00]/20"
                    : "bg-white text-[#64748b] border-[#e8edf2] hover:text-[#1A1D4E] hover:border-[#FF5B00]/30"
                }`}
              >
          ...
          {isFilterActive && (
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded-full text-[10px] sm:text-[11px] font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer border border-dashed border-[#FF5B00]/40 bg-[#FFF0E8] text-[#FF5B00] hover:bg-[#EF3C23] hover:text-white hover:border-[#FF5B00] select-none whitespace-nowrap outline-none"
            >
      ...
      <div className="flex flex-col sm:flex-row items-stretch border border-[#e8edf2] rounded-[12px] overflow-hidden bg-white mb-6 shadow-[0_4px_18px_rgba(26,29,78,0.05)]">
        ...
        <div className="w-full sm:w-[48%] bg-white p-5 sm:p-6 flex flex-col justify-between flex-shrink-0 text-left min-w-0 border-t sm:border-t-0 sm:border-l border-[#e8edf2]">
          <div>
            <p className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.12em] text-[#FF5B00] mb-2">
              <span className="inline-block w-1 h-3 rounded-full bg-[#FF5B00]" />
      ...
      {!hasHybridFeed ? (
        <div className="w-full text-center py-10 bg-[#F8FAFC] border border-[#e8edf2] rounded-[12px] text-[#8a9bb0] flex flex-col items-center gap-3">
          ...
          {isFilterActive && (
            <button
              onClick={clearPlatformFilter}
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-wider bg-white border border-[#FF5B00]/40 text-[#FF5B00] hover:bg-[#EF3C23] hover:text-white transition-all cursor-pointer select-none"
            >
              <X size={11} className="shrink-0" />
        ...
        <div className="mt-5 p-5 bg-[#F8FAFC] border border-[#e8edf2] rounded-[12px] anim-fade text-left space-y-4">
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4 text-xs font-sans">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-white border border-[#e8edf2] rounded-[8px] px-3 py-2 text-[#1A1D4E] focus:outline-none focus:border-[#FF5B00] transition-colors"
                />
              </div>
              <div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button 
                type="submit"
                className="bg-[#FF5B00] hover:bg-[#FF6B00] text-white font-semibold py-2 px-5 rounded-[8px] transition-colors shadow-sm shadow-[#FF5B00]/20 cursor-pointer"
              >
                Add Creator Review
              </button>
            </div>
      ...
      {canAddCreatorReview && !showAddForm && (
        ...
        >
          <Plus size={14} className="text-[#FF5B00]" />
          Add creator review
        </button>
      )}
