    ...
    <div className="flex-grow p-6 space-y-6 max-w-6xl">
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-[#e8edf2] rounded-lg">
          ...
          <Link to={createPath} className="mt-4 inline-block text-xs font-bold uppercase text-[#FF5B00] hover:underline">
            Create content
          </Link>
        </div>
      ) : view === 'grid' ? (
...
function ContentCard({ item, onClick }: { item: MarketingContentRecord; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="text-left bg-white border border-[#e8edf2] rounded-lg p-4 hover:border-[#FF5B00]/30 transition-colors"
    >
