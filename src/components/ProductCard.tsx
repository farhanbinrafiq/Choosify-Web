      ...
      >
        ...
        >
            ...
            <ProductCardImageCarousel 
              images={getProductCardImages(product)} 
              alt={product.title} 
              containerClassName="absolute inset-0 p-2.5"
            />
        </div>
        
        <div className="flex flex-col gap-1.5 pt-2 flex-1 text-left min-h-0">
          <div className="flex items-center justify-between gap-1 w-full">
            ...
            <div className="flex items-center gap-0.5 shrink-0 ml-auto bg-gray-50 px-1 py-0.5 rounded">
              ...
              <span className="text-[8px] font-semibold text-gray-500">4.8</span>
            </div>
          </div>
          
          {mode === 'wholesale' && (
            <div className="flex flex-wrap gap-1">
              <span className="bg-orange-primary/10 text-[#E8500A] text-[6.5px] font-semibold px-1 rounded uppercase tracking-wider">
                Wholesale Approved
              </span>
            </div>
          )}

          <h4 className="text-[11px] font-medium text-[#1a1a2e] leading-snug line-clamp-1 group-hover:text-orange-primary transition-colors mb-1">
            {product.title}
          </h4>
          ...
          <div className="flex items-center justify-between gap-2 mt-auto pt-2 border-t border-gray-100 w-full select-none">
             <div className="flex flex-col text-left justify-center min-w-0">
                <span className="text-xs font-mono font-semibold text-[#E8500A]">BDT {product.price.toLocaleString()}</span>
                {product.originalPrice && (
                  <span className="text-[8px] font-mono text-gray-400 line-through mt-0.5">৳{product.originalPrice}</span>
                )}
             </div>
      ...
      <div className="relative h-[135px] w-full bg-gray-50 overflow-hidden flex items-center justify-center p-2.5 rounded-lg shrink-0">
        ...
        <div className="absolute top-2 left-2 z-25 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex gap-1">
          <button 
            onClick={toggleSave}
            className={cn(
              "w-6 h-6 rounded-full border flex items-center justify-center bg-white border-gray-100 shadow-none transition-transform hover:scale-105",
              isSaved ? "text-[#E8500A]" : "text-gray-400 hover:text-[#E8500A]"
            )}
          >
             <Bookmark size={10} className={isSaved ? "fill-current" : ""} />
        ...
        {product.tag && (
          <div className={cn("absolute top-2 right-2 text-white text-[7.5px] font-semibold px-2 py-0.5 rounded uppercase z-25", product.tagColor || "bg-[#E8500A]")}>
            {product.tag}
          </div>
        )}
      </div>
      
      <div className="pt-2 flex flex-col flex-grow min-h-0 text-left justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1 w-full justify-between">
            <span className="text-[#E8500A] font-medium uppercase text-[8px] tracking-wider truncate max-w-[100px]">{brandName}</span>
            <div className="flex items-center gap-0.5 shrink-0 ml-auto bg-gray-50 px-1 py-0.5 rounded">
              <Star size={7.5} className="fill-orange-primary text-orange-primary" />
              <span className="text-[8px] font-semibold text-gray-500">4.8</span>
            </div>
            {mode === 'wholesale' && product.moq && (
              <span className="bg-gray-100 text-gray-600 text-[7px] font-semibold px-1 rounded uppercase tracking-wider shrink-0 ml-1">
                MOQ {product.moq}
              </span>
            )}
          </div>
        ...
        <div className="mt-auto pt-2 border-t border-gray-100 flex items-center justify-between gap-1.5">
          <div className="flex flex-col min-w-0">
             <span className="text-[7.5px] font-medium text-gray-400 uppercase tracking-widest leading-none mb-1">
               {mode === 'wholesale' ? 'Bulk Price' : 'Market Price'}
              </span>
              <span className="text-[12.5px] font-mono font-semibold text-[#E8500A] leading-none whitespace-nowrap">
