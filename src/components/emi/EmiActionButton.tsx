  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? 'border-[#FF5B00] bg-[#FFF0E8] text-[#FF5B00]'
          : 'border-[#e8edf2] bg-white text-gray-600 hover:border-[#FF5B00]/40 hover:text-[#EF3C23]',
  ...
  return (
    <div className="mt-2 p-3 bg-[#fafbfc] border border-[#e8edf2] rounded-[5px] text-left space-y-2">
      <p className="text-xs text-gray-700 leading-relaxed flex gap-2">
        <Sparkles size={14} className="text-[#FF5B00] shrink-0 mt-0.5" aria-hidden />
        {suggestion}
      </p>
      {why && <p className="text-[10px] text-gray-400"><strong className="text-gray-500">Why:</strong> {why}</p>}
      {applyHint && <p className="text-[10px] text-[#FF5B00]/80">{applyHint}</p>}
      {onDismiss && (
