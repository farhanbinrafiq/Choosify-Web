  ...
  if (!seriesItem) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        ...
        <Link to="/spotlight/explore?tab=series" className="mt-4 inline-block text-[#FF5B00] text-sm font-bold uppercase">
          Browse series
        </Link>
      </div>
    );
      ...
      <ol className="space-y-3 mb-10">
        {seriesItem.episodes.map((ep) => (
          <li key={ep.episodeId} className="flex items-center gap-3 p-3 border border-[#e8edf2] rounded-[5px]">
            <span className="text-xs font-black text-[#FF5B00] w-8">E{ep.episodeNumber}</span>
