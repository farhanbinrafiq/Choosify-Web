        ...
        {contentRows.map((row) => (
          <div key={row.contentId} className="bg-white border border-[#e8edf2] rounded-xl p-4 space-y-3">
            <div className="flex flex-wrap justify-between gap-2">
              <div>
                <button type="button" className="text-sm font-black text-navy hover:text-[#EF3C23] text-left" onClick={() => row.href && navigate(row.href)}>
                  {row.title}
                </button>
        ...
        {campaignRows.map((row) => (
          <div key={row.campaignId} className="bg-white border border-[#e8edf2] rounded-xl p-4 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <button type="button" className="text-sm font-black text-navy uppercase hover:text-[#EF3C23]" onClick={() => navigate(`/marketing/intelligence/campaigns/${row.campaignId}`)}>
                  {row.name}
                </button>
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-[#1A1A2E] tracking-tight flex items-center gap-2">
            <BarChart3 size={24} className="text-[#FF5B00]" />
        ...
        <Link to="/marketing/studio" className="inline-flex items-center gap-1 text-[10px] font-bold uppercase text-gray-400 hover:text-[#EF3C23]">
      ...
      {entityId && (
        <div className="bg-[#050514] text-white rounded-lg px-4 py-2 text-xs font-bold uppercase">
          ...
          <Link to={`/marketing/intelligence/${section === 'mission_control' ? '' : section}`} className="ml-3 text-[#FF5B00] hover:underline">← Back</Link>
        </div>
      )}
