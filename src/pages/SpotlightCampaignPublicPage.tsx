  ...
  if (!content) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        ...
        <Link to="/spotlight" className="inline-flex items-center gap-1 mt-6 text-[#FF5B00] text-sm font-bold uppercase">
      ...
      {(content.isLive || content.contentType === 'live') && (
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2.5 min-h-[44px] bg-[#FF5B00] text-white text-xs font-black uppercase tracking-wider rounded hover:bg-[#EF3C23]"
          >
            {content.live?.status === 'replay' || content.live?.status === 'ended' ? 'Watch Replay' : content.live?.status === 'upcoming' ? 'Notify Me' : 'Watch Live'}
          </Link>
          <Link
            to={`/spotlight/${content.slug}`}
            className="inline-flex items-center gap-1.5 px-4 py-2.5 min-h-[44px] border border-[#e8edf2] text-[#1a1a2e] text-xs font-bold uppercase tracking-wider rounded hover:border-[#FF5B00]/40"
          >
            Campaign Overview
          </Link>
        </div>
