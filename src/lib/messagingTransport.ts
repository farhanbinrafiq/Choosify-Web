 ...
 * client-side onSnapshot listener subscribes to) is genuinely live, or
 * whether the server has fallen back to its memory-disk backend (no
 * Firebase Admin credentials configured) — in which case an onSnapshot
...
async function fetchTransport(): Promise<MessagingTransport> {
  try {
    const res = await fetch(`${API_BASE}/messaging/persistence-mode`);
