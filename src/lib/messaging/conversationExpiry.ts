 ...
 *
 * Keep in sync with choosify-admin-4.0/shared/messaging/conversationExpiry.ts
 */
...
export interface ConversationExpiryResult {
  ...
  showWarning?: boolean;
  warningLabel?: string;
  /** Persistent notice shown while the conversation is still open (not the urgent X-hours warning). */
  freezeNotice?: string;
  closedLabel?: string;
...
/** 11:59:59.999 PM Asia/Dhaka on the given calendar date (UTC+6). */
