...
/** Which page this CTA renders on -- must have a matching entry in the shared placement registry (ctaPlacementRegistry.ts). */
...
/**
 * Reuses the existing authenticated-role/session lookup (isLoggedIn +
 * operationsApi.getSellerStatus) already used by BecomeCreatorSidebarCard --
 * not a new segmentation engine. "none" always shows.
 */
...
export interface CtaBannerItem {
  ...
  /**
   * Internal: an app route (e.g. "/suggest-brand"). External: a full https://
   * URL. Unused (empty) for creator_signup / seller_signup / none.
   */
