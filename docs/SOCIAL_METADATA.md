...
3. **`api/og.tsx`** — Vercel Edge function using `@vercel/og` `ImageResponse` to generate **1200×630** branded cards (products, brands, categories, deals, articles, creators).
4. **`api/share.ts`** — Server-rendered HTML with complete social meta for crawlers that do not execute SPA JavaScript.
5. **`middleware.ts`** — Detects Facebook, LinkedIn, X/Twitter, WhatsApp, Slack, Discord, Telegram, etc., and rewrites them to `/api/share`.
